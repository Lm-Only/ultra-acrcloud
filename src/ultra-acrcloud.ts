...
export class ACRCloudRecognizer {
    ...
    private buildStringToSign(method: string, uri: string, timestamp: number): string {
     ...
     */
    private buildMultipartBody(boundary: string, fields: Record<string, string | number>, fileBuffer: Buffer): Buffer {
        const chunks: Buffer[] = [];
        const crlf = '\r\n';
    ...
    public identify(audioSample: Buffer): Promise<ACRCloudResponse> {
        return new Promise((resolve, reject) => {
            ...
            const req = https.request(options, (res) => {
                ...
                res.on('end', () => {
                    ...
                    try {
                        resolve(JSON.parse(responseData) as ACRCloudResponse);
                    } catch (e) {
